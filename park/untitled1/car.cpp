...
string  car::car_plate()
{
    ...
    str[i] = a[rand() % (200)];
    num[i] = to_string(10000 + (int)(rand() % (60000 + 10000 - 1)));//我设置的是10000-60000之间的随机数，有需可以自己改动
    plate[i] = str[i] + num[i];
...
void car::setStarttime(){
    starttime = clock();

}
void car::setPk(int carpk)
{
    pk = carpk;
    setStarttime();
...
string car::msg1(){
    string str = "car "+ returnBd() + " in queue\n";
    return  str;
}
string car::msg2(){
    string str = "car " + returnBd() + " in the park " + std::to_string(pk +1) +"\n";
    return str;

}
string car::msg3(){
    string str = "car " + returnBd() + " go out, pay $" +std::to_string(((int)cartime/15000)*1.5) + "use time"+to_string(cartime/1000)+" min\n";
    ofstream outfile("D:\\homework_cxd\\park\\untitled1\\data.txt", ios::app);
     outfile << plate+"\t"+to_string(cartime)+"\t"+to_string((int)(cartime/15000)*1.5)<<endl;
     outfile.close();
    return str;
}
